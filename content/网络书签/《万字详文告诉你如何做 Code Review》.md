...
```
// Get 获取IPfunc (i *IPGetter) Get(cardName string) string { i.l.RLock() ip, found := i.m[cardName] i.l.RUnlock() if found {  return ip } i.l.Lock() var err error ip, err = getNetIP(cardName) if err == nil {  i.m[cardName] = ip }  i.l.Unlock() return ip}
```
...
```
if !needContinue { doA() return} else { doB() return}
```

```
if !needContinue { doA() return}doB()return
```
